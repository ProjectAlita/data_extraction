...
def lead_time_distribution_jira(df_issues: pd.DataFrame) -> pd.DataFrame:
    ...
    required_columns = ['issue_key', 'issue_type', 'created_date', 'fromString', 'toString', 'changelog_date',
                       'status', 'project_key', 'request_type']
    optional_columns = ['team']
    
    # Select only columns that exist in the DataFrame
    columns_to_select = [col for col in required_columns if col in df_issues.columns]
    columns_to_select.extend([col for col in optional_columns if col in df_issues.columns])
    
    df_filtered = df_issues[columns_to_select][
