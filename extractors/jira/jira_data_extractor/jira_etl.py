...
class ExtractJiraIssuesToDb:
    ...
    def run_sprints_pipeline(self, sprints_table: str, credentials: Optional[dict] = None) -> None:
        """Run the pipeline to extract sprints data from Jira and save it to the DataBase."""
        jira = connect_to_jira(self.cloud_provider, self.cloud_conf_path, credentials=credentials)
        db_jira = DBEngine(self.cloud_conf_path, self.cloud_provider)
    ...
    def _extract_updated_issues(self, project: str, custom_fields: dict, date_after: str, credentials: Optional[dict] = None) -> pd.DataFrame:
        """
    ...
    def _define_deleted_issues(self, project: str, ids_in_database: list, credentials: Optional[dict] = None) -> list:
